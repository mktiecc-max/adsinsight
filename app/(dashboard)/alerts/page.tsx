...
export default function AlertsPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/report/alerts", { signal: controller.signal, cache: "no-store" })
      .then(async (response) => {
    ...
    <div className="page alerts-page">
      <div className="page-heading alerts-heading">
        <div>
          ...
          <p>
            Mỗi cảnh báo gồm bằng chứng, trung vị để so và hành động cụ thể — không báo chung chung.
          </p>
        </div>
      ...
      <div className="alert-filter-bar">
        ...
        <div>
          {filters.map(([id, label]) => (
            <button key={id} onClick={() => setFilter(id)} className={cn(filter === id && "active")}>
      ...
      <div className="alert-list">
        {visible.map((alert) => {
          ...
          return (
            <article
              className={cn("alert-card card", meta.className, isSnoozed && "snoozed")}
              key={alert.id}
            >
              ...
              <div className="alert-card-actions">
                {isSnoozed ? (
                  <span className="snoozed-label"><Check size={13} /> Đã bỏ qua đến 31/07</span>
                ) : null}
                <Link href={`/performance?detail=${alert.id === "a1" ? "campaign-tuyen-sinh-he" : alert.id === "a2" ? "campaign-tu-duy-toan" : "campaign-qua-tang-he"}`} className="button button-small">
      ...
      <section className="disabled-rules">
        <div className="disabled-rules-icon"><BellOff size={18} /></div>
        <div>
          <strong>3 quy tắc chưa chạy được</strong>
          <p>Bão hòa tệp · Creative mòn · Đấu giá đắt lên</p>
          <small>
            Cần bổ sung vào nguồn quảng cáo: <code>impressions</code>, <code>clicks</code>,{" "}
            <code>frequency</code>
          </small>
        </div>
        <Info size={15} />
