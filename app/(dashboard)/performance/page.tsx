...
const deepColumns = ["cpm", "ctr", "frequency", "matchRate"];
...
function DetailDrawer({
  ...
  const relevantLeads = leads.filter(
    (lead) =>
      lead.campaign.toLowerCase().includes(row.brand) ||
      lead.ad.toLowerCase().includes(row.name.split("/").at(-1)?.trim() ?? ""),
  );
    ...
    <div className="drawer-layer" role="dialog" aria-modal="true" aria-label={`Chi tiết ${row.name}`}>
      ...
      <aside className="detail-drawer">
        ...
        <div className="drawer-zone">
          ...
          <span>{zoneMeta[row.zone].action}</span>
        </div>
        ...
        <div className="drawer-body">
          ...
          {tab === "daily" ? (
            <div>
              <div className="drawer-section-title">Xu hướng 14 ngày gần nhất</div>
              <div className="mini-bars">
                {trendData.slice(-14).map((item, index) => (
                  <div key={item.day}>
                    <span style={{ height: `${25 + (item.sql / 23) * 65}%` }} />
                    {index % 3 === 0 ? <small>{item.day.split("/")[0]}</small> : null}
                  </div>
                ))}
              </div>
              <p className="drawer-help">Cột thể hiện SQL theo ngày; kỳ này giảm rõ ở 5 ngày cuối.</p>
            </div>
          ) : null}
          ...
          {tab === "leads" ? (
            <div>
              <div className="drawer-section-title">Lead gần nhất</div>
              <div className="drawer-lead-list">
                {(relevantLeads.length ? relevantLeads : leads.slice(0, 5)).slice(0, 5).map((lead) => (
                  <div key={lead.id}>
                    <span className="num">0{lead.phone}</span>
                    <span>{lead.date.slice(0, 5)}</span>
                    <b>Bậc {lead.rank}</b>
                  </div>
                ))}
              </div>
...
export default function PerformancePage() {
  return (
    <Suspense fallback={<div className="performance-loading" aria-label="Đang tải bảng hiệu suất" />}>
      <PerformanceContent />
    </Suspense>
  );
}
